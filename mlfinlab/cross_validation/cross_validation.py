...
class PurgedKFold(KFold):
    ...
    def split(self,
        ...
        for start_ix, end_ix in test_ranges:
            ...
            end = self.samples_info_sets.iloc[end_ix-1] + (delta * self.embargo)
...
class PurgedSplit(KFold):
    # No embargo needed because just 1 train/test split (embargo needed after test)
    def __init__(self,
                 samples_info_sets: pd.Series = None,
                 test_size_pct = .25,
                 ):
        ...
        if not isinstance(samples_info_sets, pd.Series):
            raise ValueError('The samples_info_sets param must be a pd.Series')
        # super(PurgedSplit, self).__init__(1, shuffle=False, random_state=None)
        self.samples_info_sets = samples_info_sets
    ...
    def split(self,
        ...
        indices: np.ndarray = np.arange(X.shape[0])
        test_ranges: [(int, int)] = [(int(len(X) * (1-self.test_size_pct))+1, len(X))]
