...
class CombinatorialPurgedKFold(KFold):
    ...
    def split(self,
        ...
        for test_splits in combinatorial_test_ranges:
            ...
            test_times = pd.Series(index=[self.samples_info_sets.index[ix[0]] for ix in test_splits], data=[
                self.samples_info_sets[ix[1] - 1] if ix[1] + 1 >= X.shape[0] else self.samples_info_sets[ix[1] - 1] + embargo
